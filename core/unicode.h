...
#ifndef JSONLANG_UNICODE_H
#define JSONLANG_UNICODE_H

/** Substituted when a unicode translation format encoding error is encountered. */
#define JSONLANG_CODEPOINT_ERROR 0xfffd
#define JSONLANG_CODEPOINT_MAX 0x110000
...
{
    if (x >= JSONLANG_CODEPOINT_MAX)
        x = JSONLANG_CODEPOINT_ERROR;
...
static inline char32_t decode_utf8(const std::string &str, size_t &i)
{
    ...
    if ((c0 & 0x80) == 0) { //0xxxxxxx
        ...
        if (i+1 >= str.length()) {
            return JSONLANG_CODEPOINT_ERROR;
        }
        char c1 = str[++i];
        if ((c1 & 0xC0) != 0x80) {
            return JSONLANG_CODEPOINT_ERROR;
