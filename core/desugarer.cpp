...
BuiltinDecl jsonlang_builtin_decl(unsigned long builtin)
{
    switch (builtin) {
...
static constexpr char STD_CODE[] = {
    #include "std.jsonlang.h"
};

/** Desugar Jsonlang expressions to reduce the number of constructs the rest of the implementation
 * needs to understand.
 *
...
class Desugarer {
    ...
    void desugar(AST *&ast_, unsigned obj_level)
    {
        if (auto *ast = dynamic_cast<Apply*>(ast_)) {
            ...
            if (ast->tokenKind != LiteralString::BLOCK) {
                ast->value = jsonlang_string_unescape(ast->location, ast->value);
            }
    ...
    void desugarFile(AST *&ast, std::map<std::string, VmExt> *tlas)
    {
        ...
        Tokens tokens = jsonlang_lex("std.jsonlang", STD_CODE);
        AST *std_ast = jsonlang_parse(alloc, tokens);
        desugar(std_ast, 0);
        auto *std_obj = dynamic_cast<DesugaredObject*>(std_ast);
        if (std_obj == nullptr) {
            std::cerr << "INTERNAL ERROR: std.jsonlang not an object." << std::endl;
            std::abort();
        }
        ...
        for (unsigned long c=0 ; c <= max_builtin ; ++c) {
            const auto &decl = jsonlang_builtin_decl(c);
            Identifiers params;
        ...
        if (tlas != nullptr) {
            ...
            for (const auto &pair : *tlas)
            {
                ...
                if (pair.second.isCode) {
                    // Now, implement the std library by wrapping in a local construct.
                    Tokens tokens = jsonlang_lex("tla:" + pair.first, pair.second.data.c_str());
        ...
        // local std = (std.jsonlang stuff); ast
        ast = make<Local>(
            ast->location,
            EF,
...
void jsonlang_desugar(Allocator *alloc, AST *&ast, std::map<std::string, VmExt> *tlas)
{
