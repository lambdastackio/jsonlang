...
{
    try {
        std::list<Token> tokens = jsonlang_lex("test", snippet);
        Allocator allocator;
        AST* ast = jsonlang_parse(&allocator, tokens);
        (void)ast;
    } catch (StaticError& e) {
        ASSERT_TRUE(false)
...
{
    // TODO(jsonlang-team): The following error output differs from the Go
...
{
    // TODO(jsonlang-team): The error output of the following differs from the Go
    // implementation, which is:
...
TEST(Parser, TestInvalidAssert)
{
    ...
    // TODO(jsonlang-team): The error output of this differs from the Go
    // implementation, which is:
