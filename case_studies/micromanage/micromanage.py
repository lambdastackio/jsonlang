...
ext_vars = {}  # For Jsonlang evaluation
search_paths = [  # Where we look for imported jsonlang files
    os.path.dirname(os.path.realpath(__file__)) + '/lib/'
]
debug = False

#  Returns content if worked, None if file not found, or throws an exception
def jsonlang_try_path(dir, rel):
    if not rel:
...
def jsonlang_import_callback(dir, rel):
    full_path, content = jsonlang_try_path(dir, rel)
    if content:
        return full_path, content
    for path in search_paths:
        full_path, content = jsonlang_try_path(path, rel)
        if content:
            return full_path, content
...
def config_load(filename, ext_vars):
    try:
        text = _jsonlang.evaluate_file(
            filename,
            max_trace=100,
            ext_vars=ext_vars,
            import_callback=jsonlang_import_callback)
