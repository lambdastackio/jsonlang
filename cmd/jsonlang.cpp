...
{
    o << "Jsonlang commandline interpreter " << jsonlang_version() << std::endl;
}
...
void usage(std::ostream &o)
{
    version(o);
    o << "Typical Usage:\n";
    o << "jsonlang [<cmd>] {<option>} <filename>\n";
    ...
    o << "Provide a value as Jsonlang code:\n";
    ...
    o << "jsonlang fmt {<option>} <filename>\n";
    o << "and <option> can be:\n";
    o << "  -h / --help             This message\n";
    o << "  -e / --exec             Treat filename as code\n";
    o << "  -o / --output-file <file> Write to the output file rather than stdout\n";
    o << "  -i / --in-place         Update the Jsonlang file in place.  Same as -o <filename>\n";
    ...
    o << "Note that since filenames and jsonlang programs can begin with -, it is advised to\n";
    o << "use -- if the argument is unknown, e.g. jsonlang -- \"$FILENAME\".";
    o << std::endl;
}
...
/** Parse the command line arguments, configuring the Jsonlang VM context and
 * populating the JsonlangConfig.
 */
static bool process_args(int argc,
                         const char **argv,
                         JsonlangConfig *config,
                         JsonlangVm *vm) {
    ...
    for (; i<args.size() ; ++i) {
        ...
        if (arg == "-h" || arg == "--help") {
            ...
            if (arg == "-s" || arg == "--max-stack") {
                ...
                jsonlang_jpath_add(vm, dir.c_str());
            // TODO(dcunnin): Remove deprecated --var, --env and -E
                ...
                if (!get_var_val(next_arg(i, args), var, val))
                    return EXIT_FAILURE;
                jsonlang_ext_var(vm, var.c_str(), val.c_str());
            ...
            // TODO(dcunnin): Remove deprecated --code-var, --code-env
                ...
                if (!get_var_val(next_arg(i, args), var, val))
                    return EXIT_FAILURE;
                jsonlang_ext_code(vm, var.c_str(), val.c_str());
            // TODO(dcunnin): Remove deprecated --code-file
                ...
                if (!get_var_val(next_arg(i, args), var, val))
                    return EXIT_FAILURE;
                jsonlang_tla_var(vm, var.c_str(), val.c_str());
                ...
                if (!get_var_val(next_arg(i, args), var, val))
                    return EXIT_FAILURE;
                jsonlang_tla_code(vm, var.c_str(), val.c_str());
                ...
                jsonlang_gc_growth_trigger(vm, v);
            ...
            if (arg == "-i" || arg == "--in-place") {
                ...
                jsonlang_fmt_max_blank_lines(vm, l);
                ...
                jsonlang_fmt_pad_arrays(vm, true);
            } else if (arg == "--no-pad-arrays") {
                jsonlang_fmt_pad_arrays(vm, false);
            } else if (arg == "--pad-objects") {
                jsonlang_fmt_pad_objects(vm, true);
            } else if (arg == "--no-pad-objects") {
                jsonlang_fmt_pad_objects(vm, false);
            } else if (arg == "--pretty-field-names") {
                jsonlang_fmt_pretty_field_names(vm, true);
            } else if (arg == "--no-pretty-field-names") {
                jsonlang_fmt_pretty_field_names(vm, false);
            } else if (arg == "--debug-desugaring") {
                jsonlang_fmt_debug_desugaring(vm, true);
            } else if (arg.length() > 1 && arg[0] == '-') {
                std::cerr << "ERROR: Unrecognized argument: " << arg << std::endl;
                return EXIT_FAILURE;
            } else {
...
/** Reads Jsonlang code from the input file or stdin into the input buffer. */
static bool read_input(JsonlangConfig* config, std::string* input) {
...
static bool write_multi_output_files(JsonlangVm* vm, char* output,
                                     const std::string& output_dir)
{
    // If multiple file output is used, then iterate over each string from
    // the sequence of strings returned by jsonlang_evaluate_snippet_multi,
...
static bool write_output_stream(JsonlangVm* vm, char* output)
{
    // If YAML stream output is used, then iterate over each string from
    // the sequence of strings returned by jsonlang_evaluate_snippet_stream,
...
int main(int argc, const char **argv)
{
    try {
        ...
        // Evaluate input Jsonlang and handle any errors from Jsonlang VM.
        int error;
        char *output;
        switch (config.cmd) {
            case EVAL: {
                if (config.evalMulti) {
                    output = jsonlang_evaluate_snippet_multi(
                        vm, config.inputFile.c_str(), input.c_str(), &error);
                } else if (config.evalStream) {
                    output = jsonlang_evaluate_snippet_stream(
                        vm, config.inputFile.c_str(), input.c_str(), &error);
                } else {
                    output = jsonlang_evaluate_snippet(
                        vm, config.inputFile.c_str(), input.c_str(), &error);
                }
            ...
            case FMT: {
                ...
                output = jsonlang_fmt_snippet(vm, config.inputFile.c_str(), input.c_str(), &error);
